...
def create_booking(db: Session, booking: schemas.BookingBase) -> models.Booking:
    ...
    for count_day in range(booking.number_of_nights):
        db_booking = models.Booking(
            guest_name=booking.guest_name, unit_id=booking.unit_id,
            check_in_date=booking.check_in_date + datetime.timedelta(count_day), number_of_nights=booking.number_of_nights)
        db.add(db_booking)
    db.commit()
